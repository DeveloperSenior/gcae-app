const { createFolders,
    ...
    unzipFile } = require('../../src/utilities/IOUtil');
const fs = require('fs');
...
describe("IOUtil Utility", () => {
    ...
    it('should unzip in directory', async () => {

        // set up unZip
        decompress.mockReturnValue(true);
        fs.rmSync.mockImplementation(() => { });
        await unzipFile('mockPath', 'mockFolder');
        // make your assertion
        expect(decompress).toHaveBeenCalled();
        expect(fs.rmSync).toHaveBeenCalled();
    });

    it('should unzip in directory without debug', async () => {
    ...
    it('should exeption unzip file in directory', async () => {
        // set up rmSync
        fs.rmSync.mockImplementation(() => { throw { message: 'Mock rmSync Error' } });
        try {
            await unzipFile('mockPath', 'mockFolder');
        } catch (error) {
            expect(error).toBeInstanceOf(DefaultException);
            expect(error.exception).toBe('Mock rmSync Error');
        }
    });
    ...
    it('should create Read Stream', () => {
        // set up createReadStream
        fs.createReadStream.mockImplementation(() => {
            return {
                on: jest.fn((method, fn) => fn())
            }
        });
        createReadStreamFile('mockPath');
        // make your assertion
