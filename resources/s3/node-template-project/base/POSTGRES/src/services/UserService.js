...
const { DATE_FORMAT,HTTP_CODE } = require('../utilities/Constants');
...
const UserService = userRepository => {
    ...
    const login = async (user) => {
        ...
        try {
            ...
            if (!userResponse) {
                const exeption = new DefaultException('Authentication failed: Email not exist.');
                exeption.code = HTTP_CODE.UNAUTHORIZED;
                throw exeption
            ...
            if (!accessTokenMatch) {
                const exeption = new DefaultException('Authentication failed: Incorrect password.');
                exeption.code = HTTP_CODE.UNAUTHORIZED;
                throw exeption;
            ...
            const exeption = new DefaultException(error.message)
            exeption.code = HTTP_CODE.UNAUTHORIZED;
            throw exeption;
        }
    };
