...
module "ECS-Gcae-API" {
  ...
  vpc_id=var.vpc_id
  ecs_name="ambiente-gcae-api-pdn"
  ecs_task_name = "gcae-apiTsk"
  ecs_task_container_name="gcae-apisrv"
  ecs_task_image_name="${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/gecae-api:betaPdn"
  ecs_task_cpu=408
  ...
  ecs_service_name="gcae-apipdn-services"
  ecs_number_of_tasks="1"
  #ecs_load_balancer  = var.ecs_load_balancer
  #ecs_dns_namespace="local"
  ecs_service_discovery_name="gcae-apipdn-services"
