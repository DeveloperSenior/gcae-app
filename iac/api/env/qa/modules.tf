...
module "ECS-Gcae-API" {
  ...
  vpc_id=var.vpc_id
  ecs_name="ambiente-gcae-api-qa"
  ecs_task_name = "gcae-apiTsk"
  ecs_task_container_name="gcae-apisrv"
  ecs_task_image_name="${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/gecae-api:betaqa"
  ecs_task_cpu=408
